...
[CreateAssetMenu(menuName = "EventTrigger/Capture Intel On Wanted Level")]
public class EventTrigger_CaptureIntelOnWantedLevel : EventTriggerBase {
	...
	{
		if (GameManager.instance.game.intelInPlay.Count > 0 && GameManager.instance.game.player.currentWantedLevel >= m_wantedLevel) {
	...
	{
		Debug.Log ("Event Triggered: Capturing Intel");
		...
		for (int i = 0; i < m_numAgents; i++) {
			...
			if (GameManager.instance.game.agentOrganization.agentsByState.ContainsKey (GameManager.instance.game.agentOrganization.agentAIState_Idle)) {
				...
				if (validAgents.Count > 0) {
					...
					aw.ChangeAIState (GameManager.instance.game.agentOrganization.agentAIState_CaptureIntel);
				}
			}
		}
	}
